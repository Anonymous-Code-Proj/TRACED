...
ABS_VALUE_TYPE_FINE_NUMBER2ID = {str(v): i for i, v in ABS_VALUE_TYPE_FINE_NUMBER.items()}
...
class DataCollatorForValue(DataCollatorForLanguageModeling):
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        # Handle dict or lists with proper padding and conversion to tensor.
        input_ids = [example["input_ids"] for example in examples]
        attention_mask = [example["attention_mask"] for example in examples]
        value_labels = [example["value_labels"] for example in examples]
        batch = {}
        ...
        batch["value_labels"] = _torch_collate_batch(value_labels, self.tokenizer, pad_to_multiple_of=self.pad_to_multiple_of)
        batch["value_labels"][batch["attention_mask"]==0] = -100
...
def compute_metrics_quantized_value(tokenizer, inputs, predictions, references):
    true_predictions = []
    true_labels = []
    sample_true_predictions = []
    sample_true_labels = []

    for input, pred, label in zip(inputs, predictions, references):
        ...
        for idx, lb in enumerate(label):
            if lb != -100:
                token = tokenizer._convert_id_to_token(input[idx])
                if token.startswith("Ġ"): # This implementation is specifically for RoBerTa Tokenizer
                        true_pred.append(pred[idx])
                        true_lb.append(lb)

        true_predictions += true_pred
        true_labels += true_lb
        sample_true_predictions.append(true_pred)
        sample_true_labels.append(true_lb)

    return true_predictions, true_labels, sample_true_predictions, sample_true_labels
...
def main():
    ...
    model = RobertaForValue.from_pretrained(
        args.model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        ignore_mismatched_sizes=args.ignore_mismatched_sizes,
        num_value=len(ABS_VALUE_TYPE_FINE_NUMBER)
    )
    ...
    def preprocess_function(examples):
        ...
        for i in range(total):
            ...
            if len(set(abstract_values_labels_i)) == 1:
                continue
            else:
        ...
        total = len(src_seq)
        tokenized_src = tokenizer(src_seq, truncation=True, padding=padding, max_length=args.max_seq_length - args.max_input_value_length)
            ...
            for id in tolist(tokenized_src['input_ids'][i]):
                if id == tokenizer.pad_token_id or id == tokenizer.sep_token_id or id == tokenizer.cls_token_id or id == tokenizer.unk_token_id:
                    ...
                    if abstract_values_labels_i[vva_idx] == "##":
                        ...
                        abstract_value_lb.append(ABS_VALUE_TYPE_FINE_NUMBER[abstract_values_labels_i[vva_idx]])
                        
            assert len(abstract_value_lb) == len(tokenized_src['input_ids'][i]), f"No. {i} sample has different length between labels and input_ids before prepending entry variables."

            abstract_value_labels.append(abstract_value_lb)

        tokenized_entry = tokenizer(entry_variables, truncation=True, max_length=args.max_input_value_length, padding=padding)
        ...
        features = {"input_ids": tokenized_src_with_inputs["input_ids"], "attention_mask": tokenized_src_with_inputs["attention_mask"], "value_labels": abstract_value_labels}
    ...
    if not args.only_eval:
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            for step, batch in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
                ...
                inputs, predictions, references, logits = accelerator.gather((batch["input_ids"], predictions, batch["value_labels"], logits))
                ...
                true_predictions, true_labels, sample_true_predictions, sample_true_labels = compute_metrics_quantized_value(tokenizer, inputs, predictions, references)
            ...
            logger.info(f"epoch {epoch} --- eval_loss: {eval_loss}, eval_sample_acc: {eval_metric['sample_accuracy']}, eval_var_acc: {eval_metric['accuracy']}")
    ...
    if args.output_dir is not None:
        ...
        dev_data_after_filter = []
        for d in dev_data:
            if len(set(d["abstract_value_seq"].split())) != 1:
                dev_data_after_filter.append(d)
        assert len(dev_data_after_filter) == len(sample_predictions) == len(sample_labels), f"{len(dev_data)}, {len(sample_predictions)}, {len(sample_predictions)}"
        with open(os.path.join(args.output_dir, "eval_details.jsonl"), "w") as f:
            for d, pred, label in zip(dev_data_after_filter, sample_predictions, sample_labels):
                l = {}
                l['prediction'] = [ABS_VALUE_TYPE_FINE_NUMBER2ID[str(p)] for p in pred]
                l['label'] = [ABS_VALUE_TYPE_FINE_NUMBER2ID[str(l)] for l in label]
                for k, v in d.items():
                    l[k] = v
